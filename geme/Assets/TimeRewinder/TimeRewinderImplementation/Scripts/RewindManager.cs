...
public class RewindManager : MonoBehaviour
{
    ...
    public void StartRewindTimeBySeconds(float seconds)
    {
        ...
        if (rewindSeconds >= RewindManager.Instance.HowManySecondsAvailableForRewind)
        {
            Debug.Log("Not enough available rewind time. Available: " + RewindManager.Instance.HowManySecondsAvailableForRewind);
            return;  // Early exit if not enough time
        }




        Debug.Log("StartRewindTimeBySeconds called with seconds: " + seconds);
        if (IsBeingRewinded)
        {
    ...
    private void Awake()
    {
        ...
        Debug.Log($"Found {_rewindedObjects.Count} rewindable objects");  // Check how many objects are tracked
    ...
    {   
        if (IsBeingRewinded)
        {
            Debug.Log("Rewinding in progress for " + rewindSeconds + " seconds");
        ...
        else 
        {
            ...
            if (TrackingEnabled)
            {
                //Debug.Log("Tracking Enabled. Available time for rewind: " + HowManySecondsAvailableForRewind);
